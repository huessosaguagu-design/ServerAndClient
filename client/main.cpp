...
static void doConnect() {
    ...
    std::thread([]() {
        // Build register payload: [role:1byte] [name_len:4bytes] [name]
        ...
        // The relay sends back MSG_REGISTER with assigned ID via poll
        // We'll get it from the first poll in the Receiver, but we need
        // the ID now. Do one manual poll to get it.
        // Actually — the ID is sent as first message via /poll, and
        // the Receiver will handle it. For now set a placeholder.
        g_clientId = 1;  // will be updated by first message
        g_connected = true;
        g_connecting = false;
